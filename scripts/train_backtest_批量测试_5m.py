...
功能说明：
1. 交易逻辑：
   - 所有开仓信号在当日开盘时买入，当日收盘时平仓（每个信号单独测试）
   - 每次名义仓位 = 历史日结后的最高总资金 * PEAK_PERCENT（默认100%）
   - 允许杠杆（不限制 trade_size 相对于当前资金）
   - 若平仓后资金 <= 0，视为爆仓（资金归零），记录并终止回测

2. 数据处理：
   - 输入：项目根目录 data/merged/btc/btc_5m 下的 parquet 文件
   - 输出：results/backtest_批量测试 下的 CSV/PNG（包含年化收益率，每个信号单独输出）
...
# 交易配置
TRADE_DIRECTION = "多"  # 交易方向 ("多" 或 "空")
STAR_COL = "星宿"       # 信号类型列名

# 回测配置
INITIAL_CAPITAL = 1000.0  # 初始资金（USDT）
PEAK_PERCENT = 1          # 杠杆比 (名义仓位 = 历史日结后的最高总资金 * PEAK_PERCENT)
...
try:
    ...
    star_first_occurrence = {}
    for star in unique_stars:
        star_first_occurrence[star] = df[df[STAR_COL] == star].index[0]
    ...
    for index, TARGET_STAR in enumerate(all_xiuxiu, 1):
        ...
        for start_idx in segment_starts:
            ...
            # 计算名义仓位：历史日结后的最高资金 * PEAK_PERCENT
