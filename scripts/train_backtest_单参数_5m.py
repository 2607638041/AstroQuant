...
csv_trades = OUT_DIR / f"回测_单信号记录_{TRADE_DIRECTION}.csv"
...
plt.title(f'回测资金曲线 - 单信号策略 - {TRADE_DIRECTION}单', fontsize=16)
...
# 添加初始资金参考线
plt.axhline(y=INITIAL_CAPITAL, color='r', linestyle='--', alpha=0.7, label=f'初始资金: {INITIAL_CAPITAL} USDT')
...
plt.legend()
plt.tight_layout()
capital_curve_path = OUT_DIR / f"回测资金曲线_单信号策略_{TRADE_DIRECTION}单.png"
...
if not trades_df.empty:
    ...
    print("\n" + Fore.MAGENTA + f"总交易次数:   {len(trades)}" + Style.RESET_ALL)
    ...
    if liquidated:
        print(Fore.RED + "注意：回测期间发生至少一次爆仓（资金被清零），回测在爆仓当日之后停止模拟交易。" + Style.RESET_ALL)
else:
    print("\n" + "="*50)
    print(Fore.YELLOW + "           回测结果统计" + Style.RESET_ALL)
    print("="*50)
...
summary = {
    ...
    "覆盖年数": years if 'years' in locals() else 0.0,
    "交易次数": len(trades),
    "盈利次数": int(wins),
...
print(Fore.GREEN + f"已保存逐笔交易明细: {csv_trades}" + Style.RESET_ALL)
